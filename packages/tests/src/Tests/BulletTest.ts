...
export class BulletTest extends Test {
    ...
    public Step(settings: Settings): void {
        ...
        if (b2_toiStats.calls > 0) {
            ...
            g_debugDraw.DrawString(5, this.m_textLine, `toi calls = ${b2_toiStats.calls}, ave toi iters = ${(b2_toiStats.iters / b2_toiStats.calls).toFixed(1)}, max toi iters = ${b2_toiStats.maxRootIters}`);
            this.m_textLine += DRAW_STRING_NEW_LINE;

            // g_debugDraw.DrawString(5, this.m_textLine, "ave toi root iters = %3.1f, max toi root iters = %d",
            g_debugDraw.DrawString(5, this.m_textLine, `ave toi root iters = ${(b2_toiStats.rootIters / b2_toiStats.calls).toFixed(1)}, max toi root iters = ${b2_toiStats.maxRootIters}`);
            this.m_textLine += DRAW_STRING_NEW_LINE;
        }
